...
function extractKeyPhrases(text) {
    ...
    const stopWords = new Set(['и', 'в', 'во', 'не', 'что', 'он', 'на', 'я', 'с', 'со', 'как', 'а', 'то', 'все', 'она', 'так', 'его', 'но', 'да', 'ты', 'к', 'у', 'же', 'вы', 'за', 'бы', 'по', 'только', 'ее', 'мне', 'было', 'вот', 'от', 'меня', 'еще', 'нет', 'о', 'из', 'ему', 'для', 'при', 'до', 'или', 'если', 'когда', 'где', 'это', 'этот', 'эта', 'эти', 'того', 'тем', 'тех', 'том', 'также', 'их', 'чтобы', 'может', 'быть', 'был', 'была', 'были', 'есть', 'будет', 'уже', 'даже', 'более', 'менее', 'просто', 'всего', 'всех', 'всем', 'всю', 'весь', 'вся', 'очень', 'можно', 'нужно', 'надо', 'один', 'одна', 'одно', 'одни', 'два', 'две', 'три', 'четыре', 'пять']);
    ...
    const potentialTerms = Object.keys(wordFreq).filter(word => {
        ...
        return wordContext[word]?.size > 3 && wordFreq[word] > 1 && wordFreq[word] < 20;
    });
        ...
        for (const term2 of potentialTerms) {
            if (term1 !== term2) {
                ...
                if (commonSentences.length > 0) {
                    ...
                    const relationStrength = (cooccurrenceMatrix[term1]?.[term2] || 0) + 
    ...
    for (const word in wordFreq) {
        ...
        wordImportance[word] = (wordFreq[word] * contextSize * Math.sqrt(sentenceCount)) / 
...
function generateDistractors(answer, textAnalysis, count = 3) {
    ...
    if (answerPhrase) {
        ...
        contextWords.slice(0, Math.min(count, contextWords.length)).forEach(item => {
    ...
    if (distractors.size < count) {
        ...
        keyPhrases
            .filter(phrase => 
                ...
                Math.abs(phrase.importance - answerImportance) / answerImportance < 0.5 // Похожая важность
            )
    ...
    if (distractors.size < count) {
        keyPhrases
            .filter(phrase => phrase.word !== answerLower && !distractors.has(phrase.word))
...
function createQuestions(textAnalysis, count = 5) {
    ...
    function createFillInBlankQuestion() {
        ...
        const keyPhrasesWithSentences = keyPhrases.filter(p => p.sentences.length > 0);
        if (keyPhrasesWithSentences.length === 0) return null;
        
        const randomPhrase = keyPhrasesWithSentences[Math.floor(Math.random() * keyPhrasesWithSentences.length)];
        const sentenceIndex = randomPhrase.sentences[Math.floor(Math.random() * randomPhrase.sentences.length)];
    ...
    function createDefinitionQuestion() {
        ...
        const randomPhrase = importantPhrases[Math.floor(Math.random() * Math.min(5, importantPhrases.length))];
        const sentenceIndex = randomPhrase.sentences[Math.floor(Math.random() * randomPhrase.sentences.length)];
        const sentence = sentences[sentenceIndex];
        
        // Создаем вопрос о термине
        const term = randomPhrase.word.charAt(0).toUpperCase() + randomPhrase.word.slice(1);
        ...
        const similarTerms = keyPhrases
            ...
            .sort((a, b) => {
                ...
                const commonWordsA = a.context.filter(w => randomPhrase.context.includes(w)).length;
                const commonWordsB = b.context.filter(w => randomPhrase.context.includes(w)).length;
        ...
        for (const term of similarTerms) {
            ...
            const termSentenceIndex = term.sentences[Math.floor(Math.random() * term.sentences.length)];
            const termSentence = sentences[termSentenceIndex].trim();
            
            // Проверяем, что предложение не слишком похоже на правильный ответ
            if (termSentence !== answer && 
    ...
    function createRelationQuestion() {
        ...
        const relation = conceptRelations[Math.floor(Math.random() * Math.min(3, conceptRelations.length))];
        const [concept1, concept2] = relation.concepts;
        
        // Находим предложение, которое описывает связь
        const sentenceIndex = relation.sentences[Math.floor(Math.random() * relation.sentences.length)];
        const sentence = sentences[sentenceIndex];
        
        // Создаем вопрос о связи
        const formattedConcept1 = concept1.charAt(0).toUpperCase() + concept1.slice(1);
        const formattedConcept2 = concept2.charAt(0).toUpperCase() + concept2.slice(1);
        ...
        for (const otherRelation of otherRelations) {
            ...
            const otherSentenceIndex = otherRelation.sentences[Math.floor(Math.random() * otherRelation.sentences.length)];
            const otherSentence = sentences[otherSentenceIndex].trim();
            
            if (otherSentence !== answer && 
        ...
        if (distractors.length < 3) {
            ...
            const negation = answer.replace(/является|есть|представляет собой/, 'не является')
    ...
    function createComprehensionQuestion() {
        ...
        const sentenceIndex = importantSentences[Math.floor(Math.random() * importantSentences.length)];
        const sentence = sentences[sentenceIndex];
        ...
        while (distractors.length < 3) {
            ...
            words[randomIndex] = keyPhrases[Math.floor(Math.random() * keyPhrases.length)].word;
    ...
    while (remainingCount > 0) {
        const totalWeight = questionGenerators.reduce((sum, gen, i) => sum + gen.weight, 0);
    ...
    for (let i = 0; i < questionGenerators.length; i++) {
        ...
        while (questions.filter(q => q.type === generator().type).length < questionCounts[i] && attempts < maxAttempts) {
    ...
    while (questions.length < count) {
        const generators = [createFillInBlankQuestion, createDefinitionQuestion, createRelationQuestion, createComprehensionQuestion];
...
function evaluateQuestions(questions) {
    ...
    const optionLengths = questions.flatMap(q => q.options.map(o => o.length));
    
    const avgQuestionLength = questionLengths.reduce((sum, len) => sum + len, 0) / questionLengths.length;
    const avgOptionLength = optionLengths.reduce((sum, len) => sum + len, 0) / optionLengths.length;
    ...
    // Общая оценка
    score.total = (score.typeVariety + score.uniqueness + score.questionLength + score.optionLength) / 4;
...
async function initModel() {
    if (!model) {
        ...
        try {
            ...
            model = await gpt4all.createModel(modelPath);
